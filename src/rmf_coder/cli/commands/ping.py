...
from rmf_coder.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from rmf_coder.core.config import RMFConfig


def cmd_ping(config: RMFConfig) -> None:
    try:
...
async def _ping(config: RMFConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{rmf_coder.__version__}"},
    }
