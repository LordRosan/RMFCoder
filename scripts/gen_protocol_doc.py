...
def generate() -> str:
    run_id = "20260721-161020-abc123"
    ts = "2026-07-21T16:10:20.001Z"
    ...
    agent_run_req_example = {
        ...
        "method": "agent.run",
        "params": {
            "goal": "summarize the whole project",
        }
    }
    ...
    sections = [
        ...
        "## Transport\n\n",
        "- TCP loopback `127.0.0.1:7438` (override via `RMF_HOST` / `RMF_PORT`)\n",
        "- Each message is one `\\n`-terminated JSON line (NDJSON)\n",
        "- Commands use JSON-RPC 2.0 (client → server); Events use a custom `kind=event` envelope (server → client)\n\n",
        "## Commands\n\n",
